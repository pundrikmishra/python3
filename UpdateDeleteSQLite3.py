...
def del_and_update():
    ...
    # c.execute('UPDATE pm SET value = 9665 WHERE value = 2')
    # conn.commit()
    # c.execute('SELECT * FROM pm')
    # [print(row) for row in c.fetchall()]

    # c.execute('DELETE FROM pm WHERE value = 9665')
    # conn.commit()
    # print(50*'am')
    # c.execute('SELECT * FROM pm')
    # [print(row) for row in c.fetchall()]
    ...
    print(50*'km')
    c.execute('SELECT * FROM pm WHERE value =3')
    [print(row) for row in c.fetchall()]
    c.execute('SELECT * FROM pm WHERE  value =3')
    [print(len(c.fetchall()))]
    # c.execute('DELETE FROM pm WHERE value = 3 LIMIT 2 ')
    # conn.commit()
    c.execute('SELECT * FROM pm')
    [print(row) for row in c.fetchall()]
